...
(function () {
    ...
    html += '<a href="" id="idinput" class="diy import" data-type="json">导入思源文档id</a>';
    html += '<input class="diy idinput" id="siyuan"></input>',
        ...
        html += '导入<input type="file" id="fileInput" accept=".km,.txt,.md,.json,.ccmind" >',
        html += '</button>',
    ...
    添加导入 = async function () {
        ...
        idInput.addEventListener('click', async function (event) {
            ...
            res2 = await 以id获取文档块markdown(window.location.host, "", idinput.value)
            console.log(res2)
            try {
